...
const dataParticipant = class {
  ...
  static postParticipant = async (req = request, res = response) => {
    ...
    if (!result.isEmpty()) {
      ...
      const participantUniq = await Participantdata.ParticipantsOne(
        req.body.email
      );
      if (participantUniq.alert) {
        const participant = await Participantdata.InsertionParticipants(
          req.body
        );
        if (participant.success) {
          res.status(201).render("index", {
            success: "Enregistrement effectué avec success ! ",
          });
        } else {
          res
            .status(400)
            .json({ "Une erreur est surveni": participant.erreur });
        }
      } else {
        res
          .status(201)
          .render("index", { alert: "Participant(e) existe déjà ! " });
      }
    }
  };
  ...
  static postNotifications = async (req = request, res = response) => {
    ...
    if (status === "ACCEPTED") {
      ...
      return res
        .status(200)
        .redirect("https://simro.info/cartdor/politique.html");
    } else {
      console.log("❌ Paiement échoué ou annulé");
      return res
        .status(400)
        .redirect("https://simro.info/cartdor/contenus.html");
    }
  };
  ...
  static getNotification = async (req = request, res = response) => {
    ...
    if (status) {
      ...
      if (status === "ACCEPTED") {
        ...
        res.redirect("https://simro.info/cartdor/?status=success"); // Remplacez par votre page de succès
      } else {
        // Paiement échoué, rediriger vers la page d'échec
        res.redirect("https://simro.info/cartdor/?status=failure"); // Remplacez par votre page d'échec
      }
    } else {
      // Si le statut ne peut pas être récupéré, rediriger vers une page d'erreur
      res.redirect("https://simro.info/cartdor/?status=failure"); // Remplacez par votre page d'erreur
    }
  };
};
